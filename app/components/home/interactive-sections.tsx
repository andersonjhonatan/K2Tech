...
export function ExperiencesSection() {
  ...
  useEffect(() => { if (window.matchMedia("(prefers-reduced-motion: reduce)").matches) return; const timer = window.setInterval(() => setReviewPage((page) => (page + 1) % reviewPages), 9000); return () => window.clearInterval(timer); }, [reviewPages]);
  function changeReviewPage(direction: number) { setReviewPage((page) => (page + direction + reviewPages) % reviewPages); }
  function sendReview(event: FormEvent<HTMLFormElement>) { event.preventDefault(); const form = new FormData(event.currentTarget); const name = form.get("reviewer")?.toString() || ""; const project = form.get("review-project")?.toString() || ""; const review = form.get("review")?.toString() || ""; setReviewSent(true); const url = whatsappLink(`Nova avaliação recebida pelo site da K2 Tech.\n\nCliente: ${name || "Não informado"}\nProjeto: ${project || "Não informado"}\n\nAvaliação:\n${review}`); const tab = window.open(url, "_blank", "noopener,noreferrer"); if (!tab) window.location.href = url; }
...
export function ContactSection() {
  ...
  function sendBrief(event: FormEvent<HTMLFormElement>) { event.preventDefault(); const form = new FormData(event.currentTarget); const name = form.get("name")?.toString() || ""; const company = form.get("company")?.toString() || ""; const message = form.get("message")?.toString() || ""; setSent(true); const url = whatsappLink(`Olá, vim pelo site da K2 Tech.\n\nNome: ${name || "Não informado"}\nEmpresa ou marca: ${company || "Não informada"}\n\nSobre o projeto:\n${message}`); const tab = window.open(url, "_blank", "noopener,noreferrer"); if (!tab) window.location.href = url; }
  return <section id="contato" className="contact container section-space"><div className="contact-top"><div className="section-label"><span>06</span><i/> VAMOS COMEÇAR</div><p>Você tem um momento.<br/>A gente dá a ele <em>presença.</em></p></div><div className="contact-grid"><h2>Vamos fazer seus<br/>convidados dizerem<br/><em>“eu preciso ir”.</em></h2><form onSubmit={sendBrief}><label>SEU NOME<input name="name" required autoComplete="name" placeholder="Como podemos te chamar?"/></label><label>EMPRESA / MARCA<input name="company" autoComplete="organization" placeholder="Qual é o seu negócio?"/></label><label>ME CONTA UM POUCO<textarea name="message" required placeholder="Qual momento você quer transformar em experiência?" rows={3}/></label><button className="button button-dark" type="submit">Enviar mensagem <Arrow /></button><p className="form-privacy">Ao enviar, você será direcionado ao WhatsApp da K2 Tech. Consulte nossa <Link href="/privacidade">Política de Privacidade</Link>.</p>{sent && <p className="form-note" role="status">Abrindo conversa no WhatsApp…</p>}</form></div></section>;
}
